...
MyLCD::MyLCD(uint8_t rs, uint8_t rw, uint8_t enable,
...
void MyLCD::init(uint8_t fourbitmode, uint8_t rs, uint8_t rw, uint8_t enable,
			 uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3,
			 uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7)
{
  _rs_pin = rs;
...
int MyLCD::read_button() {
  int reading, adc_key_in, crntBtnState, lastDebounceTime, btn_hld_time;
 ...
 // return reading;
  ...
  //Debouncing Logic
	...
	if(reading == last_state && hold_timer.is_finished() && last_state == btnUP) return btnUPHld;
	if(reading == last_state && hold_timer.is_finished() && last_state == btnDOWN) return btnDOWNHld;
	if(reading == last_state && debounce_timer.is_finished()) return reading;
	return btnNONE;
}
